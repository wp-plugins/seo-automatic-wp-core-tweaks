<?php
/*
Post Teaser (http://wordpress.org/extend/plugins/post-teaser/)
...
//]]>
	...
	function option_page() {
			...
			<form action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>" method="post" name="post_teaser_form" id="post_teaser_form">
				...
				<?php endif; ?>
				<?php _e("Show read more link at the end of teased post <i>(immediately following post text with no line-breaks)</i>", 'post-teaser') ?>
				</label></p>
				...
				<?php endif; ?>
				<?php _e("Show post statistics as block <i>(original style teaser massage with paragraph break after post text)</i>", 'post-teaser') ?>
				</label></p>

				<p><label>
				<?php if ($this->omit_fullpost == '1' && $block_stats != '1'): ?>
				...
				<?php endif; ?>
				<?php _e("Omit post statistics for full post <i>(option is disabled if block statistics above is not enabled)</i>", 'post-teaser') ?>
				</label></p>
				</fieldset>
	...
	function process($content) {
		global $post, $cookiehash, $action, $pages, $page, $multipage, $sem_home_page;
		...
		***/
		...
		if (($i + 1 < $block_count || $more) && !$showall) {
			...
			if ($this->more_link) {
				...
				if ($lastp) {
					...
					$teaser_str = " <span class=\"post-teaser-more\">" . post_teaser::placeholders($more_template) . '</span>';
		...
		if ($this->block_stats && !($this->omit_fullpost && $fullpost)) {
			$content .= "\n\n<div class=\"post-teaser-block\">" . post_teaser::placeholders($template) . '</div>';
		}
