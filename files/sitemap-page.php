...
if (($_REQUEST['action'] == "changecore") && ($_REQUEST['add_sitemap'] == "ON")) {
...
$post_date_gmt =gmdate("Y-m-d H:i:s");
  ...
  $wpdb->query("UPDATE $wpdb->posts SET guid = '" . get_permalink($post_id) . "' WHERE ID = '$post_id'");
	...
	echo "<p>The sitemap was created.</p>";
} else {
	echo "<p>The sitemap was not created. Perhaps it already exists.</p>";
}

} elseif (($_REQUEST['action'] == "changecore") && ($_REQUEST['add_sitemap'] != "ON")) {
	echo "<p>The sitemap was not created.</p>";
} else {
?>
<p><input name="add_sitemap" type="checkbox" value="ON" checked /> <?php _e('Add the SiteMap for DDSiteMap Generator.') ?></p>
<?php } 
?>