...
<?php if ($_REQUEST['action'] != "changecore") :  
	$options .= '<li><input type="hidden" name="selection" value="custom" class="tog" checked /><input type="checkbox" id="permchange" value="ON" checked onclick="setCheckedRight(\'permchange\')"> Change permalink structure to custom setting: <input type="text" size="40" name="permalink_structure" value="/%category%/%postname%/" /></li>';
